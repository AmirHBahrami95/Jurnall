...
public class User{
  ...
  public void create()throws UserException,IOException{ // for register
    ...
    if(this.exists())
      ...
      tmp.createNewFile();
			saltStr=Cryptos.bytesToHex(Cryptos.getSalt()); // XXX found ya, bitch >:
      FileUtils.writeF(
        tmp,
        saltStr
      ); 
  ...
  private boolean isPasswordValid(String passw){
    ...
    return h2!=null && !h2.isEmpty() && savedH2!=null && !savedH2.isEmpty() && h2.equals(savedH2);
  }
  ...
  private String readSavedSalt() throws IOException{
    return FileUtils.readF(this.getSavedSaltFile());
  }
